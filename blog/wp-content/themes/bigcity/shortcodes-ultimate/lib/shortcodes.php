<?php
	 ...
	 * @return string Output html
	 */
	function su_divider2_shortcode( $atts, $content = null ) {
		extract( shortcode_atts( array(
				'top' => false
				), $atts ) );

		return ( $top ) ? '<div class="su-divider2"><a href="#">' . __( 'Top', 'shortcodes-ultimate' ) . '</a></div>' : '<div class="su-divider2"></div>';
	}
	 ...
	 * @return string Output html
	 */
	function su_divider3_shortcode( $atts, $content = null ) {
		extract( shortcode_atts( array(
				'text' => '',
				'top' => false
				), $atts ) );

		return '<div class="su-divider3"><span>'.$text.'</span></div>';
	}
	 ...
	 * @return string Output html
	 */
	function featured_posts_shortcode( $atts, $content = null ) {
		extract( shortcode_atts( array(
				'cat' => '',
				'num' => '5',
				'order' => 'DESC'
     ...
     global $post;
        $myposts = get_posts('numberposts='.$num.'&order='.$order.'&orderby=post_date&cat='.$cat);

        foreach($myposts as $post) :
                setup_postdata($post);
             $retour.='<h2><a href="'.get_permalink().'">'.the_title("","",false).'</a></h2><div class="excerpt">'.apply_filters('the_excerpt',get_the_excerpt() ).'</div>';
        endforeach;

        return '<div id="blog2">' .$retour. '</div>';
		}

   
		/**
	 * Shortcode: featured posts2
	 *
	 * @param array $atts Shortcode attributes
	 * @param string $content
	 * @return string Output html
	...
	function featured_posts_shortcode2( $atts, $content = null ) {
		extract( shortcode_atts( array(
				'cat' => '',
				'num' => '5',
				'order' => 'DESC'
     ...
     global $post;
        ...
        foreach($myposts as $post) :
                setup_postdata($post);
                        global $more;
                        $more = 0;
             $retour.='<h1><a href="'.get_permalink().'">'.the_title("","",false).'</a></h1><div class="excerpt">'.apply_filters('the_content',get_the_content($more_link_text = 'Read more') ).'</div>';
        endforeach;
	...
	function featured_posts_shortcode3( $atts, $content = null ) {
		extract( shortcode_atts( array(
				'cat' => '',
				'num' => '5',
				'order' => 'DESC'
     ...
     global $post;
        ...
        foreach($myposts as $post) :
                setup_postdata($post);
                        global $more;
                        $more = 0;
             $retour.='<h1 style="margin-bottom:-10px;"><a href="'.get_permalink().'">'.the_title("","",false).'</a></h1><div class="entry-utility">Posted on'.apply_filters('the_time',get_the_time('j F Y') ).' | <a href="'.get_permalink().'">'.apply_filters('the_comments',get_comments_number()). ' comments</a></div><br/><div class="excerpt">'.apply_filters('the_content',get_the_content($more_link_text = 'Read more') ).'</div>';
        endforeach;
	...
	function su_column_shortcode( $atts, $content = null ) {
		...
		return ( $last ) ? '<div class="su-column su-column-' . $size . ' su-column-last">' . do_shortcode( $content ) . '</div><div class="su-spacer su-spacer-10"></div>' : '<div class="su-column su-column-' . $size . '">' . do_shortcode( $content ) . '</div>';
	}
...
$img_style = '';
	...
	function su_box_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-box" ><div class="su-box-title" style="background-color:' . $color . '; text-shadow:1px 1px 0 ' . $styles['text_shadow'] . '">' . $title . '</div><div class="su-box-content">' . do_shortcode( $content ) . '</div></div>';
	}
	...
	function su_note_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-note" style="background-color:' . $styles['light_color'] . '"><div class="su-note-shell" style="border:1px solid ' . $styles['dark_color'] . '">' . do_shortcode( $content ) . '</div></div>';
	}

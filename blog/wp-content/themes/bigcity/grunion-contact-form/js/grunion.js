...
FB.ContactForm = function() {
	...
	function hidePopup () {
		try {
			...
			jQuery("#TB_window", window.parent.document).fadeOut("fast",function(){jQuery('#TB_window,#TB_overlay,#TB_HideSelect', window.parent.document).trigger("unload").unbind().remove();});
	...
	function loadFieldEditor (id) {
		try {
			...
			if (fbForm.fields[id].required) {
				jQuery('#fb-new-required').attr("checked", true);
			} else {
				jQuery('#fb-new-required').attr("checked", false);
			}
		...
		try {
			...
			jQuery.post(ajaxurl, fbForm, function(response) {
				...
				if (currentCode.match(regexp)) {
					if (isVisual) {
						win.tinyMCE.activeEditor.execCommand('mceSetContent', false, currentCode.replace(regexp, response).replace(/\n/g, "<br>").split("<br>").slice(1).join("<br>"));
					} else {
						// looks like the visual editor is disabled,
						// update the contents of the post directly
						jQuery( '#content', window.parent.document ).html( currentCode.replace( regexp, response ) );
					}
				} else {
					if (isVisual) {
						win.tinyMCE.activeEditor.execCommand('mceSetContent', false, response.replace(/\n/g, "<br>").split("<br>").slice(1).join("<br>"));
					} else {
						// looks like the visual editor is disabled,
						// update the contents of the post directly
						jQuery( '#content', window.parent.document ).html( response );
					}
				}
				hidePopup();
			});
	...
	return {
		...
		init: function () {
			...
			if (jQuery('#edButtonPreview', window.parent.document).hasClass('active')) {
				var win = window.dialogArguments || opener || parent || top;

...
function grunion_post_type_columns_filter( $cols ) {
	$cols = array(
		...
		'feedback_from'		=> __( 'From' ),
		'feedback_message'		=> __( 'Message' ),
...
function grunion_manage_post_columns( $col, $post_id ) {
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			echo '<strong>';
			echo get_post_meta( $post_id, '_feedback_subject', TRUE );
			echo '</strong><br />';
			...
			if ( !empty( $extra_fields ) ) {
				...
				foreach ( (array) $extra_fields as $k => $v ) {
					echo "<tr><td align='right'><b>{$k}</b></td><td>{$v}</td></tr>\n";
				}
				echo '</table>';
			}
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Resore this item from the Trash' );
				...
				echo '">' . __( 'Restore' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently' ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently' ) . "</a></span>";
?>
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam' );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash' );
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash' ) . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam' );
...
function grunion_ajax_shortcode() {
	...
	$shortcode = '[raw][contact-form'.$atts.']';
	$shortcode .= "\n";
...
function grunion_ajax_spam() {
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam' ) . ' <span class="count">';
